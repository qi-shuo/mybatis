...
public class MapperBuilderAssistant extends BaseBuilder {
  ...
  public Cache useNewCache(Class<? extends Cache> typeClass,
      ...
      boolean blocking,
      Properties props) {
    //设置cache对象属性,currentNamespace当前的作用的namespace
    Cache cache = new CacheBuilder(currentNamespace)
            //如果type指定了会使用自定的Cache,否则就是和一级缓存一样使用默认的PerpetualCache
    ...
    //将cache负责给MapperBuilderAssistant.currentCache
    //之后会用到这个currentCache对象
    currentCache = cache;
    return cache;
  }
  ...
  public MappedStatement addMappedStatement(
    ...
    MappedStatement.Builder statementBuilder = new MappedStatement.Builder(configuration, id, sqlSource, sqlCommandType)
        ...
        .useCache(valueOrDefault(useCache, isSelect))
            //将之前生成的Cache封装到MappedStatement,所以Cache对象是namespance共用的
        .cache(currentCache);
